...
public class HackRegistry implements GameSessionEventListener {
    ...
    public final LawnmowerHack lawnmowerHack = new LawnmowerHack();
    ...
    private final TreeMap<String, Hack> hackLibrary =
            new TreeMap<>(String::compareToIgnoreCase);

    public HackRegistry(){
        try {
            for (Field field : HackRegistry.class.getDeclaredFields()) {
        ...
        WurstClient.INSTANCE.getEventManager().add(GameSessionEventListener.class, this);

    }

    public void registerHack(Hack hack) {
        this.hackLibrary.putIfAbsent(hack.getName(), hack);
    }

    public Hack getHackByName(String name) {
        return hackLibrary.get(name);
    }

    public ArrayList<Hack> getAllHax() {
        return new ArrayList<>(hackLibrary.values());
    }

    public int countHax() {
        return hackLibrary.size();
    }

    @Override
    public void onSessionChange(GameSessionEvent gameSessionEvent) {
        if (gameSessionEvent.getIsStarting()){
            WurstClient.INSTANCE.currentHackProfile.getHackList().forEach(hackName-> {
                hackLibrary.get(hackName).setEnabled(WurstClient.INSTANCE.currentHackProfile.isEnabled(hackName));
            });
        }
    }
}
