...
public final class HackList extends ProfileList implements UpdateListener {
    ...
    public HackList(String defaultEnabledHacksFile) {
        ...
        System.out.println("HACKS profilesFolder is "+ profilesFolder);
        loadProfile(defaultEnabledHacksFile);
        eventManager.add(UpdateListener.class, this);
    }

    @Override
    public void onUpdate() {
        currentEnabledHacksFile.loadHacks();
    }
    ...
    public void loadProfileHacks() {
        setHacks(currentEnabledHacksFile.loadHacks());
    }

    public void loadProfile(String fileName){
        currentEnabledHacksFile = new EnabledHacksFile(profilesFolder.resolve(fileName));
        loadProfileHacks();
        this.currentEnabledHacksFile.save(this.createJson());
    }

    private void setHacks(Set<Hack> newHacks) {
        hax.values().forEach(hack -> hack.setEnabled(false));
        newHacks.forEach(hack -> this.hax.put(hack.getName(), hack));
        currentEnabledHacksFile.save(this.createJson());
    }
    ...
    public JsonObject createJson() {
        ...
        for (Hack hack : this.getAllHax())
            json.addProperty(hack.getName(), hack.isEnabled());
        return json;
    }

    public void registerHack(Hack hack) {
        this.hax.putIfAbsent(hack.getName(), hack);
    }
