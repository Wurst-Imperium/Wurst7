...
public abstract class NavigatorScreen extends Screen
{
		...
		int x2, int y2)
	{
		...
		VertexConsumer buffer =
			RenderUtils.getVCP().getBuffer(RenderLayer.getGui());
		buffer.vertex(matrix, x1, y1, 0).color(shadowColor1).next();
		buffer.vertex(matrix, x1, y2, 0).color(shadowColor2).next();
		buffer.vertex(matrix, x2, y2, 0).color(shadowColor2).next();
