...
public final class NavigatorMainScreen extends NavigatorScreen
{
	...
	protected void onRender(int mouseX, int mouseY, float partialTicks)
	{
			...
			.size(); i++)
		{
			...
			renderFeature(mouseX, mouseY, partialTicks, i, featureX, featureY);
		}
	...
	private void renderFeature(int mouseX, int mouseY, float partialTicks,
		int i, int x, int y)
	{
		ClickGui gui = WurstClient.INSTANCE.getGui();
		float[] bgColor = gui.getBgColor();
		...
		if(!clickTimerRunning)
		{
			...
			minecraft.textRenderer.draw(buttonText, area.x + 4, area.y + 4,
				0xffffff);
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			GL11.glEnable(GL11.GL_BLEND);
		}
