...
public class TradingUtils
{
	...
	{
		AutoLibrarianHack autoLibrarianHack =
			WurstClient.INSTANCE.getHax().autoLibrarianHack;
		return autoLibrarianHack.isEnabled()
			&& autoLibrarianHack.enableGuiLessTradeDetection.isChecked();
	}
	...
	public static void clientLevelPost()
	{
		...
		Entity tradableEntity =
			TradingUtils.getCrosshairTradableEntity(minecraft, isWindowOpen);
	...
	private static void sendInteractionPacket(Entity entity, LocalPlayer player)
	{
		ServerboundInteractPacket packet =
			ServerboundInteractPacket.createInteractionPacket(entity,
				player.isShiftKeyDown(), InteractionHand.MAIN_HAND);
	...
	private static boolean isValidVillagerForTrading(Villager villager)
	{
		...
		if(player != null)
		{
			...
			return !mainHandItem.is(Items.VILLAGER_SPAWN_EGG)
				&& !mainHandItem.is(Items.NAME_TAG);
		}
	 ...
	 */
	public static Entity getCrosshairTradableEntity(Minecraft minecraft,
