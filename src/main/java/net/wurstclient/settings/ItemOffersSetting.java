...
public final class ItemOffersSetting extends Setting
{
    private final ArrayList<ItemOffer> offers = new ArrayList<>();
    private final ItemOffer[] defaultOffers;

    public ItemOffersSetting(String name, String description,
                             String... enchantments)
    {
        ...
        Arrays.stream(enchantments)
                ...
                .filter(Objects::nonNull).map(ItemOffer::create)
                .filter(ItemOffer::isValid).distinct().sorted()
                .forEach(offers::add);
        defaultOffers = offers.toArray(new ItemOffer[0]);
    }
    ...
    public int indexOf(ItemOffer offer)
    {
        return Collections.binarySearch(offers, offer);
    }

    public boolean contains(ItemOffer offer)
    {
        return indexOf(offer) >= 0;
    }

    public boolean isWanted(ItemOffer offer)
    {
    ...
    public void add(ItemOffer offer)
    {
     ...
     */
    public void replace(int index, ItemOffer offer)
    {
    ...
    {
        return new ItemOffersEditButton(this);
    }
    ...
    public void fromJson(JsonElement json)
    {
        try
        {
            ...
            wson.getAllObjects().parallelStream().map(this::loadOffer)
                    .filter(Objects::nonNull).filter(ItemOffer::isValid).distinct()
    ...
    private ItemOffer loadOffer(WsonObject wson)
    {
        try
        {
            ...
            return new ItemOffer(id, level, price);
    ...
    public JsonObject exportWikiData()
    {
        ...
        json.addProperty("type", "ItemOffers");

        JsonArray jsonDefaultOffers = new JsonArray();
        for(ItemOffer offer : defaultOffers)
        {
            JsonObject jsonOffer = new JsonObject();
            jsonOffer.addProperty("id", offer.id());
            jsonOffer.addProperty("level", offer.level());
