...
public final class CreatingTemplateState extends TemplateToolState
{
	...
	public void onUpdate(TemplateToolHack hack)
	{
		...
		for(int i = 0; i < blocksPerTick && !sortingHelper.isEmpty(); i++)
		{
			...
			{
				double dPos = last1024Added.getLast().getSquaredDistance(pos);
				if(dPos >= dCurrent)
					continue;
		...
		float partialTicks)
	{
		...
		Box bounds = BoxBackports.enclosing(start, end).contract(1 / 16.0);
		...
		List<Box> boxes = last1024Added.stream()
