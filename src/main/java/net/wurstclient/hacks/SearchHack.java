...
public final class SearchHack extends Hack
	...
	private final HashMap<Chunk, List<ChunkSearcher>> searchers = new HashMap<>();
	...
	public void onUpdate() {
		List<Block> currentBlocks = block.getBlockNames().stream()
				.map(BlockUtils::getBlockFromName)
	...
	private void removeSearchersOutOfRange(ChunkPos center, int chunkRange) {
		for (List<ChunkSearcher> searchers : new ArrayList<>(searchers.values())) {
			for (ChunkSearcher searcher : searchers) {
				if (searchers.size() < 1) {
					continue;
				}

				ChunkPos searcherPos = searchers.get(0).getChunk().getPos();
	...
	private void replaceSearchersWithDifferences(List<Block> currentBlocks, int dimensionId) {
		for (List<ChunkSearcher> oldSearchers : new ArrayList<>(searchers.values())) {
			Set<String> oldBlockNames = oldSearchers.stream()
					...
					.map(BlockUtils::getName)
					.collect(Collectors.toSet());
			Set<String> currentBlockNames = currentBlocks.stream()
					.map(BlockUtils::getName)
					.collect(Collectors.toSet());
	...
	private void replaceSearchersWithChunkUpdate(List<Block> currentBlocks,
												 int dimensionId) {
		synchronized (chunksToUpdate) {
			...
			for (Iterator<Chunk> itr = chunksToUpdate.iterator(); itr.hasNext(); ) {
				...
				List<ChunkSearcher> oldSearchers = searchers.get(chunk);
				if (oldSearchers == null) {
					continue;
				}
				for (ChunkSearcher oldSearcher : oldSearchers) {
					removeSearcher(oldSearcher);
	...
	private void addSearcher(Chunk chunk, List<Block> block, int dimensionId) {
		stopPool2Tasks();
		List<ChunkSearcher> chunkSearchers = block.stream()
				.map(b -> new ChunkSearcher(chunk, b, dimensionId))
				.peek(searcher -> searcher.startSearching(pool1))
				.collect(Collectors.toList());
		searchers.put(chunk, chunkSearchers);
	}
	...
	private boolean areAllChunkSearchersDone() {
		for (List<ChunkSearcher> searchers : searchers.values()) {
			for (ChunkSearcher searcher : searchers) {
