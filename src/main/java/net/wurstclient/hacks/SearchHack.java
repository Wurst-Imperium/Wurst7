...
public final class SearchHack extends Hack implements UpdateListener, PacketInputListener, RenderListener {
	private final BlockSetting block = new BlockSetting("Block", "The type of block to search for.",
			"minecraft:diamond_ore", false);

	private final EnumSetting<Area> area = new EnumSetting<>("Area",
			"The area around the player to search in.\n" + "Higher values require a faster computer.", Area.values(),
			Area.D11);
	...
	public void onReceivedPacket(PacketInputEvent event) {
		...
		if (packet instanceof BlockUpdateS2CPacket) {
			...
			ChunkDeltaUpdateS2CPacket change = (ChunkDeltaUpdateS2CPacket) packet;
	...
	private void addSearchersInRange(ChunkPos center, int chunkRange, Block block, int dimensionId) {
	...
	private void removeSearchersOutOfRange(ChunkPos center, int chunkRange) {
		for (ChunkSearcher searcher : new ArrayList<>(searchers.values())) {
			...
			if (Math.abs(searcherPos.x - center.x) <= chunkRange && Math.abs(searcherPos.z - center.z) <= chunkRange)
				continue;
	...
	private void replaceSearchersWithDifferences(Block currentBlock, int dimensionId) {
		for (ChunkSearcher oldSearcher : new ArrayList<>(searchers.values())) {
			if (currentBlock.equals(oldSearcher.getBlock()) && dimensionId == oldSearcher.getDimensionId())
				continue;
	...
	private void replaceSearchersWithChunkUpdate(Block currentBlock, int dimensionId) {
	...
	private void startGetMatchingBlocksTask(BlockPos eyesPos) {
		...
		Callable<HashSet<BlockPos>> task = () -> searchers.values().parallelStream()
				.flatMap(searcher -> searcher.getMatchingBlocks().stream())
				.sorted(Comparator.comparingInt(pos -> eyesPos.getManhattanDistance(pos))).limit(maxBlocks)
	...
	private HashSet<BlockPos> getMatchingBlocksFromTask() {
		...
		if (matchingBlocks.size() < maxBlocks)
			...
			ChatUtils.warning("Search found \u00a7lA LOT\u00a7r of blocks!"
					+ " To prevent lag, it will only show the closest \u00a76" + limit.getValueString()
					+ "\u00a7r results.");
			notify = false;
		}
	...
	private void startCompileVerticesTask() {
		...
		Callable<ArrayList<int[]>> task = BlockVertexCompiler.createTask(matchingBlocks);
	...
	private enum Area {
		D3("3x3 chunks", 1), D5("5x5 chunks", 2), D7("7x7 chunks", 3), D9("9x9 chunks", 4), D11("11x11 chunks", 5),
		D13("13x13 chunks", 6), D15("15x15 chunks", 7), D17("17x17 chunks", 8), D19("19x19 chunks", 9),
		D21("21x21 chunks", 10), D23("23x23 chunks", 11), D25("25x25 chunks", 12), D27("27x27 chunks", 13),
		D29("29x29 chunks", 14), D31("31x31 chunks", 15), D33("33x33 chunks", 16);
