...
public final class GlideHack extends Hack implements UpdateListener {
	private final CheckboxSetting doSneakFall = new CheckboxSetting("Temporary Disable",
			...
			true);
	private final SliderSetting fallSpeed = new SliderSetting("Fall speed", 0.125, 0.005, 0.25, 0.005,
			ValueDisplay.DECIMAL);
	
	private final SliderSetting moveSpeed = new SliderSetting("Move speed", "Horizontal movement factor.", 1.2, 1, 5,
			0.05, ValueDisplay.PERCENTAGE);
	
	private final SliderSetting minHeight = new SliderSetting("Min height",
			"Won't glide when you are\n" + "too close to the ground.", 0, 0, 2, 0.01,
	...
	public void onUpdate() {
		if (doSneakFall.isChecked() && MC.options.keySneak.isPressed())
			return;
		...
		if (minHeight.getValue() > 0) {
			...
			Stream<BlockPos> stream = StreamSupport.stream(BlockUtils.getAllInBox(min, max).spliterator(), true);
			
			// manual collision check, since liquids don't have bounding boxes
			if (stream.map(BlockUtils::getState).map(BlockState::getMaterial).anyMatch(Material::isLiquid))
				return;
		}
