...
public final class WindPearlHack extends Hack implements UpdateListener
{
	...
	// Optionally restore AutoMace after we finish (only if it was on before)
	...
	private boolean throwPearl()
	{
		Predicate<ItemStack> isPearl =
			s -> s != null && !s.isEmpty() && s.is(Items.ENDER_PEARL);
	...
	private void tryWindBurstAtFeet()
	{
		...
		Predicate<ItemStack> isWind =
			s -> s != null && !s.isEmpty() && (s.is(Items.WIND_CHARGE)
				|| s.getHoverName().getString().toLowerCase().contains("wind"));
