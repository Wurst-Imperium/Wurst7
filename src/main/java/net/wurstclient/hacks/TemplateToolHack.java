	...
	public void onRender(MatrixStack matrixStack, float partialTicks)
	{
		...
		if(template != null)
		{
			ArrayList<BlockPos> reversed =
				new ArrayList<>(template.getSortedBlocks());
			Collections.reverse(reversed);
			List<Box> boxes =
				reversed.stream().map(pos -> new Box(pos).contract(1 / 16.0))
					.limit(1024).toList();
		...
		{
			Box preview = new Box(Step.START_POS.getPos())
				.union(new Box(Step.END_POS.getPos())).contract(1 / 16.0);
