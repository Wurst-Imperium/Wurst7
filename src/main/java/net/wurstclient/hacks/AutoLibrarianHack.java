	...
	implements UpdateListener, RenderListener
{
	...
	public void onUpdate()
	{
		...
		MerchantScreen tradeScreen = (MerchantScreen)MC.currentScreen;
	...
	private void placeJobSite()
	{
		...
		Hand hand = MC.player.getMainHandStack().getItem() == Items.LECTERN
		...
		ActionResult result = MC.interactionManager.interactBlock(MC.player,
			MC.world, hand, params.toHitResult());
	...
	private void setTargetVillager()
	{
		...
		Stream<VillagerEntity> stream = StreamSupport
			.stream(MC.world.getEntities().spliterator(), true)
			.filter(e -> !e.removed).filter(e -> e instanceof VillagerEntity)
	...
	public void onRender(float partialTicks)
	{
		...
		if(jobSite != null)
			RenderUtils.drawOutlinedBox(
				new Box(jobSite).offset(-regionX, 0, -regionZ));
