	...
	implements UpdateListener, RenderListener
{
		...
		RegionPos region)
	{
		...
		buffer.vertex(x1, y, z1).color(color).normal(1, 0, 1).next();
		buffer.vertex(x2, y, z2).color(color).normal(1, 0, 1).next();
		buffer.vertex(x2, y, z1).color(color).normal(-1, 0, 1).next();
		buffer.vertex(x1, y, z2).color(color).normal(-1, 0, 1).next();
	}
}
