...
public final class CrystalAuraHack extends Hack implements UpdateListener
{
	...
	private final SliderSetting damageThreshold = new SliderSetting("Damage Threshold",
			"Only pops a crystal when the damage is\n"
					+ "above this value."
	...
	private final CheckboxSetting filterStands =
			new CheckboxSetting("Filter armor stands",
					"Won't target armor stands.\n" + "when auto-placing crystals.\n\n"
							...
							+ "target or an existing crystal.",
					true);
	private final CheckboxSetting checkDamage = new CheckboxSetting("Check Damage", "Checks if exploding a crystal\nwould kill the player exploding\nit.",false);
	...
	private void detonate(ArrayList<Entity> crystals)
	{
		...
		{
			shouldSwing = shouldSwing(e);
			if (checkDamage.isChecked() && !shouldSwing) break;
			faceBlocks.getSelected().face(e.getBoundingBox().getCenter());
			MC.interactionManager.attackEntity(MC.player, e);
		}

		if(!crystals.isEmpty())
			if (! (checkDamage.isChecked() && !shouldSwing))
				MC.player.swingHand(Hand.MAIN_HAND);
	}
	...
	{
		if (checkDamage.isChecked() && !shouldSwing(pos))
			return false;
	...
	private ArrayList<Entity> getNearbyTargets()
	{
		...
		stream = stream.filter(e -> {
			...
			for (Direction d : Direction.values()) {
				...
				if (!isClickableNeighbor(neighbor)) return false;
				return calcDamage(e,neighbor) >= damageThreshold.getValue();
			}
			return false;
		});
	...
	private boolean shouldSwing(Entity crystal) {
		boolean shouldSwing;
		//Rough calculation.
		//TODO Improve Calculation
		double crystalDamage;
		if (crystal.getY() > MC.player.getY()) crystalDamage = 0.5;
		else if (MC.player.getY() >= crystal.getY() + 10.4) crystalDamage = 0;
		else crystalDamage = 20;
		shouldSwing = MC.player.getHealth() > crystalDamage;
		return shouldSwing;
	}
	private boolean shouldSwing(BlockPos crystal) {
		boolean shouldSwing;
		//Rough calculation.
		//TODO Improve Calculation
		double crystalDamage;
	...
	private double calcDamage(Entity targetPos, BlockPos crystalPos) {
		double d;
		if (crystalPos.getY() > targetPos.getY()) d = 0.5;
		else if (targetPos.getY() >= crystalPos.getY() + 10.4) d = 0;
		else d = 20;
		return d;
	}
}
