	...
	implements UpdateListener, LeftClickListener, RenderListener
{
	...
	public void onUpdate()
	{
		...
		if(player.abilities.creativeMode)
		{
			...
			progress = 1;
			prevProgress = 1;
			breakBlocksWithPacketSpam(blocks3);
			return;
		}
		
		for(BlockPos pos : blocks2)
			if(breakOneBlock(pos))
			{
				currentBlock = pos;
				break;
			}
	...
	private boolean breakOneBlock(BlockPos pos)
	{
		Direction side = null;
	...
	private void breakBlocksWithPacketSpam(Iterable<BlockPos> blocks)
	{
		Vec3d eyesPos = RotationUtils.getEyesPos();
