...
public class NameTagFeatureRendererStorageMixin
{
	@Shadow
	@Final
	List<NameTagFeatureRenderer.Submit> nameTagSubmitsSeethrough;
	
	@Shadow
	@Final
	List<NameTagFeatureRenderer.Submit> nameTagSubmitsNormal;
	
	@WrapOperation(
		method = "add(Lcom/mojang/blaze3d/vertex/PoseStack;Lnet/minecraft/world/phys/Vec3;ILnet/minecraft/network/chat/Component;ZILnet/minecraft/client/renderer/state/level/CameraRenderState;)V",
		at = @At(value = "INVOKE",
	...
	private void wrapLabelScale(PoseStack matrices, float x, float y, float z,
		...
		int i, Component text, boolean bl, int j, CameraRenderState state)
	{
	...
	@ModifyReceiver(
		...
		at = @At(value = "INVOKE",
			...
			ordinal = 0))
	private List<NameTagFeatureRenderer.Submit> swapFirstList(
		List<NameTagFeatureRenderer.Submit> originalList, Object labelCommand)
	{
		NameTagsHack nameTags = WurstClient.INSTANCE.getHax().nameTagsHack;
	...
	@ModifyReceiver(
		...
		at = @At(value = "INVOKE",
			target = "Ljava/util/List;add(Ljava/lang/Object;)Z",
			ordinal = 1))
	private List<NameTagFeatureRenderer.Submit> swapSecondList(
		List<NameTagFeatureRenderer.Submit> originalList, Object labelCommand)
	{
		NameTagsHack nameTags = WurstClient.INSTANCE.getHax().nameTagsHack;
