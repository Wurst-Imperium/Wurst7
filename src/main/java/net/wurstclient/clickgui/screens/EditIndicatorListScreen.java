...
public final class EditIndicatorListScreen extends Screen
{
	private final Screen prevScreen;
	private final IndicatorListSetting indicatorList;
	
	private ListGui listGui;
	private TextFieldWidget indicatorNameField;
	private ButtonWidget addButton;
	private ButtonWidget removeButton;
	private ButtonWidget doneButton;
	
	private String indicatorToAdd;
	
	public EditIndicatorListScreen(Screen prevScreen, IndicatorListSetting indicatorList)
	{
	...
	public boolean mouseClicked(double mouseX, double mouseY, int mouseButton)
	{
		...
		indicatorNameField.mouseClicked(mouseX, mouseY, mouseButton);
	...
	public void tick()
	{
		indicatorNameField.tick();
		
		indicatorToAdd = indicatorNameField.getText();;
		addButton.active = indicatorToAdd != null;
	...
	private Identifier getItemIDFromField()
	{
		try
		{
			return new Identifier(indicatorNameField.getText().toLowerCase());
		...
		float partialTicks)
	{
		...
		drawCenteredString(matrixStack, client.textRenderer,
			indicatorList.getName() /*+ " (" + listGui.getItemCount() + ")"*/, width / 2,
			12, 0xffffff);
		
		indicatorNameField.render(matrixStack, mouseX, mouseY, partialTicks);
		super.render(matrixStack, mouseX, mouseY, partialTicks);
		
		matrixStack.push();
		matrixStack.translate(-64 + width / 2 - 152, 0, 0);
		
		if(indicatorNameField.getText().isEmpty() && !indicatorNameField.isFocused())
		{
			matrixStack.push();
			matrixStack.translate(0, 0, 300);
			drawStringWithShadow(matrixStack, client.textRenderer,
				"colorName_cordX_cordZ", 68, height - 50, 0x808080);
			matrixStack.pop();
		}
	...
	private static class ListGui extends ListWidget
	{
		...
		public ListGui(MinecraftClient mc, EditIndicatorListScreen screen,
			List<String> list)
		{
			...
			int y, int var4, int var5, int var6, float partialTicks)
		{
			...
			//Item item = Registry.get(string (name));
			//ItemStack stack = new ItemStack(item);
			TextRenderer fr = mc.textRenderer;

			fr.draw(matrixStack, name, x + 28, y, 0xf0f0f0);
		...
		private String renderIconAndGetName(MatrixStack matrixStack,
			String name, int x, int y, boolean large)
		{
			if(name == null)
			{
				matrixStack.push();
