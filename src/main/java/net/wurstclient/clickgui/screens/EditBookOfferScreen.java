...
public final class EditBookOfferScreen extends Screen
{
	...
	public void init()
	{
		...
		addButton(levelPlusButton = new ButtonWidget(width / 2 + 2, 110, 20, 12,
			new LiteralText("+"), b -> updateLevel(1, true)));
		levelPlusButton.active = false;
		
		addButton(levelMinusButton = new ButtonWidget(width / 2 + 26, 110, 20,
			12, new LiteralText("-"), b -> updateLevel(-1, true)));
		levelMinusButton.active = false;
		
		addButton(pricePlusButton = new ButtonWidget(width / 2 + 2, 126, 20, 12,
			new LiteralText("+"), b -> updatePrice(1, true)));
		pricePlusButton.active = false;
		
		addButton(priceMinusButton = new ButtonWidget(width / 2 + 26, 126, 20,
			12, new LiteralText("-"), b -> updatePrice(-1, true)));
		priceMinusButton.active = false;
		
		addButton(saveButton = new ButtonWidget(width / 2 - 102, height / 3 * 2,
			100, 20, new LiteralText("Save"), b -> {
		...
		addButton(cancelButton =
			new ButtonWidget(width / 2 + 2, height / 3 * 2, 100, 20,
