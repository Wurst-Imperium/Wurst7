...
public final class EditBookOffersScreen extends Screen
{
	...
	public void init()
	{
		...
		addButton(new ButtonWidget(width / 2 - 154, height - 56, 100, 20,
			new LiteralText("Add"),
			b -> client.openScreen(new AddBookOfferScreen(this, bookOffers))));
		
		addButton(editButton = new ButtonWidget(width / 2 - 50, height - 56,
			100, 20, new LiteralText("Edit"), b -> {
				boolean selected = listGui.selected >= 0
					&& listGui.selected < listGui.list.size();
				if(!selected)
					return;
				
				client.openScreen(new EditBookOfferScreen(this, bookOffers,
					listGui.selected));
			}));
		editButton.active = false;
		
		addButton(removeButton = new ButtonWidget(width / 2 + 54, height - 56,
			100, 20, new LiteralText("Remove"),
			b -> bookOffers.remove(listGui.selected)));
		removeButton.active = false;
		
		addButton(new ButtonWidget(width - 106, 6, 100, 20,
			new LiteralText("Reset to Defaults"),
			b -> client.openScreen(new ConfirmScreen(b2 -> {
				if(b2)
					bookOffers.resetToDefaults();
				client.openScreen(EditBookOffersScreen.this);
			}, new LiteralText("Reset to Defaults"),
