...
public class MicrosoftLoginManager
{
		...
		throws LoginException
	{
		...
		Session session = new Session(minecraftProfile.name,
			minecraftProfile.uuid.toString(), minecraftProfile.jwt,
	...
	public static MinecraftProfile getAccount(String email, String password)
	{
		try
		{
			XBLToken xblToken = getXBLToken(
				getAuthorizationToken(getAuthorizationCode(email, password)));
			return getMinecraftProfile(getMinecraftAccessToken(xblToken.uhs,
				getXSTSToken(xblToken.token)));
	...
	private static MinecraftProfile getMinecraftProfile(String token)
		throws LoginException
	{
		...
		{
			URL url =
				new URL("https://api.minecraftservices.com/minecraft/profile");
			HttpURLConnection httpURLConnection =
				(HttpURLConnection)url.openConnection();
	...
	private static XBLToken getXBLToken(String authorizationCode)
	{
		...
		{
			URL url =
				new URL("https://user.auth.xboxlive.com/user/authenticate");
			HttpURLConnection httpURLConnection =
				(HttpURLConnection)url.openConnection();
			...
			String uhs = object.get("DisplayClaims").getAsJsonObject()
				.get("xui").getAsJsonArray().get(0).getAsJsonObject().get("uhs")
				.getAsString();
	...
	private static String getAuthorizationCode(String email, String password)
	{
		...
		try
		{
			...
			String data = new BufferedReader(new InputStreamReader(inputStream))
				.lines().collect(Collectors.joining());
			Matcher matcher =
				Pattern.compile("sFTTag:[ ]?'.*value=\"(.*)\"/>").matcher(data);
			if(!matcher.find())
	...
	private static String microsoftLogin(String email, String password,
		String cookie, String ppft, String loginURL)
	{
		String authCode = "";
		...
		try
		{
			...
			httpURLConnection.setRequestProperty("Content-Length",
				String.valueOf(encodedDataBytes.length));
			...
			Matcher matcher = pattern.matcher(
				URLDecoder.decode(httpURLConnection.getURL().toString(),
	...
	private static String encodeMap(Map<String, String> map)
	{
		...
		for(Map.Entry<String, String> entry : map.entrySet())
		{
			...
			try
			{
				str.append(URLEncoder.encode(entry.getKey(),
